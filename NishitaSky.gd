...
@export_node_path(DirectionalLight3D) var sun_object_path
...
#@export var gradient_tint_color := Color.WHITE
#@export var compute_gradient_toggle := false

#func compute_gradient(h: float = 1000.0):
#	for point in gradient.gradient.offsets:
...
#		var dir : float = -lerp(0.0, 2.0*PI, new_i)
#		var b_sun : Basis = Basis.from_euler(Vector3(dir, 0.0, 0.0))
#		var b_sample : Basis = Basis.from_euler(Vector3(dir, deg_to_rad(0.1), 0.0))
#
#		if b_sun.z.dot(b_sample.z) > 0.999999:
#			continue
#		var sky : Vector4 = sample_sky(b_sample.z, Vector3.UP*h, b_sun.z)
#		var max_sky : Vector4 = sample_sky(Vector3.UP, Vector3.UP*h, Vector3.UP)
#		sky = sky*10.0# / max(max_sky.x, max_sky.y, max_sky.z)
#		var col : Color = Color(sky.x, sky.y, sky.z)
#		max_col = max(max_col, col.r, col.g, col.b)
#		col = saturate(col, clamp((sun_desaturation_height-b_sun.z.y)/sun_desaturation_height, 0.0, 1.0)*10.0)
#		cols.append(col)
#		poss.append(new_i)
##		gradient.gradient.add_point(new_i, col)
#
#	for i in range(sample_count):
#		var new_i : float = i/(sample_count+1.0)
#		cols[i] /= max_col
#		cols[i].r *= gradient_tint_color.r
#		cols[i].g *= gradient_tint_color.g
#		cols[i].b *= gradient_tint_color.b
#		cols[i].a = 1.0
#		if i > 0 and cols[i]==cols[i-1]:
#			continue
#		gradient.gradient.add_point(poss[i], cols[i])
#	gradient.gradient.remove_point(sample_count-1)
...
func _process(delta):
#	if compute_gradient_toggle:
#		compute_gradient_toggle = false
#		compute_gradient()
	
	var sun_object = get_node(sun_object_path)
	sky_material.set_shader_parameter("precomputed_sun_enabled", 1.0 if sun_enabled else 0.0)
	...
	sun_object.rotation.x = min(rotation.x, asin(min_sun_y))
...
#	if sun_object.rotation.x > asin(min_sun_y):
#		sun_object.rotation.x = -pow(abs(-sun_object.rotation.x+asin(min_sun_y))*0.05, 0.5)+ asin(min_sun_y)
#	else:
#		max(-abs(-sun_object.rotation.x+asin(min_sun_y)) - asin(min_sun_y), -15.0)#-sqrt(abs(sun_object.rotation.x-asin(min_sun_y)))+asin(min_sun_y)#
	
	var shader_sun_dir : Vector3 = sun_object.global_transform.basis.z
#	shader_sun_dir.y = max(shader_sun_dir.y, min_sun_y)
#	shader_sun_dir = shader_sun_dir.normalized()
	var sample_color : Vector4 = sample_sky(shader_sun_dir, Vector3.UP*sun_ground_Height, sun_dir, Vector3.ONE, Vector3.ONE)
#	var max_sun_color : Vector4 = sample_sky(Vector3(1.0, 1.0, 0.0).normalized(), Vector3.UP*sun_ground_Height, Vector3(1.0, 1.0, 0.0).normalized(), Vector3.ONE, Vector3(light_color.r, light_color.g, light_color.b))
	var max_sun_color : Vector4 = sample_sky(Vector3(1.0, 0.0, 0.0), Vector3.UP*sun_ground_Height, Vector3(1.0, 0.0, 0.0), Vector3.ONE, Vector3.ONE)

#	if sun_dir.y > 0.0:
#		sample_color = normalized_color(sample_color)
#	else:
	sample_color = sample_color / max(max_sun_color.x, max_sun_color.y, max_sun_color.z)
#	sample_color = sample_color / max(max_sun_color.x, max_sun_color.y, max_sun_color.z)
#	if max(sample_color.x, sample_color.y, sample_color.z) == 0.0:
#		sun_color = Color.BLACK
#	else:
#		sample_color = sample_color / max(sample_color.x, sample_color.y, sample_color.z)
#		sun_color = Color(sample_color.x, sample_color.y, sample_color.z)
	sun_color = Color(sample_color.x, sample_color.y, sample_color.z).srgb_to_linear()
	sun_color = saturate(sun_color, pow(clamp((sun_desaturation_height-sun_dir.y)/sun_desaturation_height, 0.0, 1.0),0.25) )
	sample_color *= Vector4(light_color.r, light_color.g, light_color.b, 1.0)
	sun_object.light_color = sun_color
...
#	if (sun_object.global_transform.basis.z.y < 1.0-cos( deg_to_rad(sun_disk_scale) )):
#		sun_object.hide()
#	else:
#		sun_object.show()
	...
	#	if cloud_sun_rot.x > asin(min_sun_y):
	#		cloud_sun_rot.x = -pow(abs(-cloud_sun_rot.x+asin(min_sun_y))*0.05, 0.5)+ asin(min_sun_y)
		var cloud_sun_dir := Basis.from_euler(cloud_sun_rot).z

		var sun_sample_color : Vector4 = sample_sky(cloud_sun_dir, Vector3.UP*cloud_height, sun_dir, Vector3.ONE, Vector3.ONE)
		
		var max_shader_sun_color : Vector4 = sample_sky(Vector3(1.0, 0.0, 0.0), Vector3.UP*cloud_height, Vector3(1.0, 0.0, 0.0), Vector3.ONE, Vector3.ONE)
	#
	#	if cloud_sun_dir.y > 0.0:
	#		sun_sample_color = normalized_color(sun_sample_color)
	#	else:

		if max(max_shader_sun_color.x, max_shader_sun_color.y, max_shader_sun_color.z) == 0.0:
			sun_sample_color = sun_sample_color
		else:
			sun_sample_color = sun_sample_color / max(max_shader_sun_color.x, max_shader_sun_color.y, max_shader_sun_color.z)
		
		var sun_sample_color_normalized : Color = Color(sun_sample_color.x, sun_sample_color.y, sun_sample_color.z).srgb_to_linear()
		sun_sample_color_normalized = saturate(sun_sample_color_normalized, pow(clamp((sun_desaturation_height-sun_dir.y)/sun_desaturation_height, 0.0, 1.0),0.25))
		sun_sample_color_normalized *= Color(light_color.r, light_color.g, light_color.b, 1.0)
		
		var cloud_ambient_color_vector_max : Vector3 = atmosphere(Vector3(1.0, 1.0, 0.0).normalized(), Vector3.UP * cloud_height, Vector3.UP)[0]
		var cloud_ambient_color_vector : Vector3
		if max(cloud_ambient_color_vector_max.x, cloud_ambient_color_vector_max.y, cloud_ambient_color_vector_max.z) == 0.0:
			cloud_ambient_color_vector = Vector3.ZERO
		else:
			cloud_ambient_color_vector = atmosphere(Vector3(1.0, 1.0, 0.0).normalized(), Vector3.UP * cloud_height, sun_dir)[0] / max(cloud_ambient_color_vector_max.x, cloud_ambient_color_vector_max.y, cloud_ambient_color_vector_max.z)
		var cloud_ambient_color : Color = Color(cloud_ambient_color_vector.x, cloud_ambient_color_vector.y, cloud_ambient_color_vector.z)

		sun_sample_color.w = 1.0
		
		sky_material.set_shader_parameter("precomputed_Atmosphere_sun", sun_sample_color_normalized)
		sky_material.set_shader_parameter("precomputed_Atmosphere_ambient", cloud_ambient_color)
		sky_material.set_shader_parameter("precomputed_Atmosphere_ground", sun_color)

var rayleigh : float = 2.0
#var Vector3 rayleigh_color : source_color = Vector3(5.8, 13.0, 22.4, 1.0)
#var float mie : hint_range(0, 1) = 0.005
#var float mie_eccentricity : hint_range(-1, 1) = 0.8
#var Vector3 mie_color : source_color = Vector3(0.69, 0.729, 0.812, 1.0)

var turbidity : float = 1.0
var ground_color : Vector3 = Vector3(0.1, 0.07, 0.034)
var ground_brightness : float = 1.0

#var sampler2D night_sky : source_color, hint_default_black
#
